    ...
    <>
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ? 'bg-[#FAF8F5]/95 dark:bg-[#0B1617]/95 backdrop-blur-md shadow-sm border-b border-[#C5A880]/20 py-3'
            : 'bg-[#FAF8F5]/80 dark:bg-[#0B1617]/80 backdrop-blur-sm py-4 border-b border-stone-200/50 dark:border-stone-800/60'
        }`}
      >
            ...
            {navLinks.map((link) => (
              link.isAction ? (
                <button
                  key={link.name}
                  onClick={link.action}
                  className={`transition-colors relative py-1 link-luxury ${
                    link.isCms
                      ? 'px-2.5 py-0.5 rounded-full bg-[#8C6239]/10 text-[#8C6239] dark:text-[#C5A880] border border-[#C5A880]/30 hover:bg-[#8C6239]/20 font-semibold'
                      : activeView === 'catalog' && link.name === 'Shop Catalog'
                      ? 'text-[#8C6239] dark:text-[#C5A880] font-bold'
                      : 'hover:text-[#132629] dark:hover:text-white'
                  }`}
                >
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-2.5">
            ...
            >
              <div className="relative">
                ...
                {totalCartCount > 0 && (
                  <span className="absolute -top-2 -right-2 w-4 h-4 rounded-full bg-gradient-to-r from-[#8C6239] to-[#C5A880] text-white text-[9px] font-bold flex items-center justify-center shadow">
                    {totalCartCount}
                  </span>
                )}
              </div>
              {subtotal > 0 && (
                <span className="hidden sm:inline text-xs font-bold text-[#8C6239] dark:text-[#C5A880]">

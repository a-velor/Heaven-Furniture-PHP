...
export const CustomCursor = () => {
  ...
  return <div
    ...
    aria-hidden="true"
    className={`pointer-events-none fixed inset-0 z-[99999] transition-opacity duration-300 ${isVisible && !isTextInput ? "opacity-100" : "opacity-0"}`}
  >
      {
  ...
  >
        <div
    className={`flex items-center justify-center rounded-full transition-all duration-150 ease-out ${isHovered ? "w-12 h-12 border border-[#C5A880] bg-[#C5A880]/15 dark:bg-[#C5A880]/20 shadow-[0_0_20px_rgba(197,168,128,0.35)]" : "w-7 h-7 border border-[#C5A880]/80 dark:border-[#C5A880]/90 bg-transparent shadow-[0_0_10px_rgba(197,168,128,0.15)]"} ${isClicked ? "scale-75 opacity-90" : "scale-100 opacity-100"}`}
  >
          {hoverLabel && <span className="text-[9px] uppercase tracking-widest font-semibold text-[#132629] dark:text-[#FAF8F5] bg-[#C5A880]/90 px-1.5 py-0.5 rounded shadow-sm">
              {hoverLabel}
            </span>}
        </div>
      </div>
  ...
  >
        <div
    className={`rounded-full bg-[#C5A880] shadow-[0_0_8px_rgba(197,168,128,0.9)] transition-all duration-100 ease-out ${isHovered ? "w-2 h-2 scale-125" : "w-1.5 h-1.5 scale-100"} ${isClicked ? "scale-50" : ""}`}
  />
      </div>
    </div>;
};

...
interface BespokeStudioProps {
  onOpenQuote: (roomType?: string) => void;
}

export const BespokeStudio: React.FC<BespokeStudioProps> = ({ onOpenQuote }) => {
  const [selectedRoom, setSelectedRoom] = useState('living');
  const [selectedWood, setSelectedWood] = useState(WOOD_SPECIES[0].id);
  const [selectedFinish, setSelectedFinish] = useState('bouclé');
  ...
  const finishOptions = [
    { id: 'bouclé', name: 'Textured Belgian Bouclé', tone: 'Warm Oatmeal' },
...
Could we arrange a complimentary consultation or review floor measurements?`;
  ...
  return (
    <section id="bespoke" className="py-20 lg:py-28 bg-[#FAF8F5] dark:bg-[#0B1617] text-[#2C221E] dark:text-[#F2EFE9] relative transition-colors duration-300">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {roomOptions.map((room) => (
                  <button
                    ...
                    className={`px-3 py-2 text-xs font-semibold uppercase tracking-wider transition-all text-center border ${
                      selectedRoom === room.id
                        ? 'bg-[#C5A880] text-[#132629] border-[#C5A880]'
                        : 'bg-[#1B3236] dark:bg-[#162528] text-[#FAF8F5]/80 border-transparent hover:border-[#C5A880]/40'
                    }`}
                  >
                    ...
                    <button
                      ...
                      className={`p-3.5 text-left border transition-all flex flex-col justify-between ${
                        selectedWood === wood.id
                          ? 'border-[#8C6239] dark:border-[#C5A880] bg-[#8C6239]/10 dark:bg-[#C5A880]/10 ring-1 ring-[#8C6239] dark:ring-[#C5A880]'
                          : 'border-stone-200 dark:border-stone-700 hover:border-stone-400 dark:hover:border-stone-500 bg-white dark:bg-[#162528]'
                      }`}
                    >
                      <div className="flex items-start justify-between">
                        ...
                        {selectedWood === wood.id && (
                          <Check className="w-4 h-4 text-[#8C6239] dark:text-[#C5A880]" />
                        )}
                      </div>
                    ...
                    <button
                      ...
                      className={`p-3 text-left border text-xs transition-all ${
                        selectedFinish === finish.id
                          ? 'border-[#132629] dark:border-[#C5A880] bg-[#132629] dark:bg-[#C5A880] text-[#FAF8F5] dark:text-[#132629]'
                          : 'border-stone-200 dark:border-stone-700 hover:border-stone-400 dark:hover:border-stone-500 bg-[#FAF8F5] dark:bg-[#162528] text-stone-800 dark:text-stone-200'
                      }`}
                    >

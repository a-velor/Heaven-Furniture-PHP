...
import { X, Search, CheckCircle2, Circle, Clock, Phone, PackageCheck } from "lucide-react";
  ...
  >
    ...
    /* Quick selector buttons */
            ...
            <div className="flex items-center gap-2 overflow-x-auto text-[11px]">
              ...
              {orders.slice(0, 4).map((ord) => <button
    key={ord.id}
    onClick={() => {
      setInputCode(ord.id);
      setSearchedId(ord.id);
    }}
    className={`px-2.5 py-1 rounded font-mono shrink-0 transition-colors border ${currentOrder?.id === ord.id ? "bg-[#8C6239] text-white border-[#8C6239]" : "bg-stone-200/60 dark:bg-stone-800 text-stone-700 dark:text-stone-300 border-stone-300 dark:border-stone-700 hover:border-stone-400"}`}
  >
    ...
    /* Dynamic Timeline */
              ...
              <div className="bg-stone-100/70 dark:bg-stone-950/60 p-5 rounded-xl border border-stone-200 dark:border-stone-800">
                ...
                <div className="relative pl-6 space-y-6 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-stone-300 dark:before:bg-stone-700">
                  {currentOrder.trackingTimeline.map((step, idx) => <div key={idx} className="relative">
                      {
    /* Circle icon */
  }
                      <span className={`absolute -left-6 top-0.5 flex items-center justify-center w-5 h-5 rounded-full ${step.done ? "text-emerald-600 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-950" : "text-stone-400 bg-stone-200 dark:bg-stone-800"}`}>
    ...
    /* Items in this Order */
              ...
              <div className="p-4 rounded-xl bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 space-y-2.5">
                ...
                <div className="divide-y divide-stone-100 dark:divide-stone-800">
                  {currentOrder.items.map((item, i) => <div key={i} className="py-2.5 flex items-center justify-between gap-3 text-xs">
                      <div className="flex items-center gap-3">
    ...
    /* Atelier Support Direct Contact */
            ...
            </div> : <div className="text-center py-12 text-stone-400">

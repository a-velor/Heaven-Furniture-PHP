...
import { X, Star, Heart, Check, ShieldCheck, Ruler, Truck, Sparkles, Send } from "lucide-react";
...
export const ProductDetailModal = () => {
  ...
  const handleReviewSubmit = (e) => {
    ...
    setRevComment("");
    setRevLocation("");
    setTimeout(() => setReviewSubmitted(false), 4e3);
  };
  return <div className="fixed inset-0 z-[110] overflow-y-auto">
      {
    /* Backdrop */
  }
      <div
  ...
  />
                  {selectedProduct.highlightTag && <span className="absolute top-3 left-3 px-3 py-1 bg-[#132629]/90 dark:bg-[#C5A880]/90 text-white dark:text-[#0B1617] text-[10px] font-bold tracking-widest uppercase rounded">
  ...
  >
    ...
    /* Thumbnails */
                ...
                {allImages.length > 1 && <div className="flex gap-2 overflow-x-auto pb-1">
                    {allImages.map((img, idx) => <button
    key={idx}
    onClick={() => setActiveImageIndex(idx)}
    className={`relative w-16 h-16 rounded-md overflow-hidden border-2 shrink-0 transition-all ${activeImageIndex === idx ? "border-[#C5A880] ring-1 ring-[#C5A880]" : "border-transparent opacity-60 hover:opacity-100"}`}
  >
                ...
                <div className="flex items-center gap-4 mt-2 mb-4">
                  <div className="flex items-center gap-1 text-amber-500">
                    ...
                    <span className="text-xs font-bold text-stone-800 dark:text-stone-200">
                      {(selectedProduct.rating || 5).toFixed(1)}
                    </span>
    ...
    /* Pricing */
                ...
                <div className="flex items-baseline gap-3 p-3 bg-stone-100/60 dark:bg-stone-900/60 rounded-lg border border-stone-200 dark:border-stone-800 mb-5">
                  ...
                  {selectedProduct.originalPrice && selectedProduct.originalPrice > selectedProduct.price && <span className="text-sm line-through text-stone-400 dark:text-stone-500">
                      {formatPrice(selectedProduct.originalPrice)}
                    </span>}
                  {selectedProduct.originalPrice && <span className="ml-auto text-[10px] font-semibold uppercase tracking-wider bg-rose-100 dark:bg-rose-950/40 text-rose-600 dark:text-rose-400 px-2 py-0.5 rounded">
    ...
    className={`pb-2 mr-6 transition-colors border-b-2 ${activeTab === "details" ? "border-[#8C6239] dark:border-[#C5A880] text-stone-900 dark:text-stone-100" : "border-transparent text-stone-400 hover:text-stone-600 dark:hover:text-stone-300"}`}
  >
                    Craftsmanship & Specs
                  </button>
                  <button
    onClick={() => setActiveTab("reviews")}
    className={`pb-2 transition-colors border-b-2 ${activeTab === "reviews" ? "border-[#8C6239] dark:border-[#C5A880] text-stone-900 dark:text-stone-100" : "border-transparent text-stone-400 hover:text-stone-600 dark:hover:text-stone-300"}`}
  >
                    Client Reviews ({reviews.length})
                  </button>
                </div>

                {activeTab === "details" ? <div className="space-y-4 text-xs text-stone-600 dark:text-stone-300">
                    <p className="leading-relaxed">{selectedProduct.description}</p>

                    {
    /* Wood Finish Options */
  }
                    {selectedProduct.woodFinishOptions && selectedProduct.woodFinishOptions.length > 0 && <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedProduct.woodFinishOptions.map((w) => <button
    key={w}
    type="button"
    onClick={() => setSelectedWood(w)}
    className={`px-3 py-1.5 rounded text-[11px] border transition-all ${currentWood === w ? "border-[#8C6239] dark:border-[#C5A880] bg-[#8C6239]/10 text-stone-900 dark:text-stone-100 font-semibold" : "border-stone-300 dark:border-stone-700 hover:border-stone-400"}`}
  >
                              {w}
                            </button>)}
                        </div>
    ...
    /* Upholstery Options */
                    ...
                    {selectedProduct.upholsteryOptions && selectedProduct.upholsteryOptions.length > 0 && <div>
                        <label className="block font-semibold text-stone-800 dark:text-stone-200 mb-1.5">
                          Upholstery / Top Material: <span className="text-[#8C6239] dark:text-[#C5A880] font-normal">{currentFabric}</span>
                        </label>
                        <div className="flex flex-wrap gap-2">
                          {selectedProduct.upholsteryOptions.map((u) => <button
    key={u}
    type="button"
    onClick={() => setSelectedFabric(u)}
    className={`px-3 py-1.5 rounded text-[11px] border transition-all ${currentFabric === u ? "border-[#8C6239] dark:border-[#C5A880] bg-[#8C6239]/10 text-stone-900 dark:text-stone-100 font-semibold" : "border-stone-300 dark:border-stone-700 hover:border-stone-400"}`}
  >
                              {u}
                            </button>)}
                        </div>
  ...
  />
    ...
    >
                      ...
                      {reviewSubmitted && <p className="text-[11px] text-emerald-600 font-medium">Thank you! Your review has been recorded.</p>}
                    </form>
      ...
      /* Review List */
                    ...
                    <div className="space-y-3">
                      {reviews.length === 0 ? <p className="text-xs text-stone-500 italic">No reviews yet for this piece. Be the first to review!</p> : reviews.map((rev) => <div key={rev.id} className="border-b border-stone-200 dark:border-stone-800 pb-2.5">
                            ...
                            <div className="flex items-center gap-1 text-amber-500 my-0.5">
                              {Array.from({ length: rev.rating }).map((_, i) => <Star key={i} className="w-3 h-3 fill-amber-500" />)}
                              {rev.location && <span className="text-[10px] text-stone-400 ml-1">({rev.location})</span>}
                            </div>
